...
create.metaginfo.object <- function(fpgs, species, basal.value = 0.5){
  ...
  #colnames(genes.vals.05) <- names(pathigraph.genes)

  meta.05 <- NULL
  meta.05$pathigraphs <- fpgs
  meta.05$all.labelids <- labelids
  results.05 <- hipathia::hipathia(genes.vals.05, meta.05, test = FALSE)
  results.dec.05 <- hipathia::hipathia(genes.vals.05, meta.05, decompose = TRUE,
                             test = FALSE)

  # covert from Experiment class to numeric
  eff.norm <- assay(results.05)[, 1]

  path.norm <- assay(results.dec.05)[, 1]
  ...
  metaginfo$path.norm <- path.norm
  metaginfo$eff.norm <- eff.norm
