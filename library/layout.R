...
refine.layout <- function(gg, coords, fixed, e = 10e-7, maxiter = 500, w = 0.5,
  ...
  p0 <- stats::quantile(dists[dists > 0], p0q, na.rm=T)[[1]]
  p00 <- p0 * p0mult
...
add.param <- function(param, graph, newgraph, common, init = TRUE){
  if(init==T){
...
add.functions.to.pathigraph <- function(pathigraph, entrez2hgnc, dbannot,
                                        maxiter = 500, verbose = TRUE, w = 10,
                                        e = 0.001, use.last.nodes = TRUE,
                                        use.edges = FALSE, p0q = 0.01,
                                        p0mult = 5){
  ...
  if(nrow(nff) == 0){
    ...
    rl <- refine.layout(newgraph, lay, fixed, w = w, maxiter = maxiter, e = e,
                        use.edges = use.edges, p0q = p0q, p0mult = p0mult)
    rl$iter
    ...
    newpathigraph$effector.subgraphs_funs <-
        create.effector.subgraphs(newpathigraph, funs = TRUE)
...
add.functions.to.pathigraphs <- function(apgs, entrez2hgnc, dbannot,
                                         maxiter = 100, p0mult = 4){
  ...
  fpgs <- lapply(apgs,function(x) {
    ...
    cat(x$path.name)
    cat("\n")
    add.functions.to.pathigraph(x, entrez2hgnc, dbannot, maxiter = maxiter,
